...
HTML_EXPANDABLE_ERROR = (
    '<div class="verbose expandable"><span class="error2">&lt;--- %s <span'
    ' class="marker">[+]</span></span><div class="content">%s</div></div>\n'
)
HTML_EXPANDABLE_INCONCLUSIVE = (
    '<div class="verbose expandable"><span class="inconclusive2">&lt;--- %s <span'
    ' class="marker">[+]</span></span><div class="content">%s</div></div>\n'
)
...
def filter_button(enabled_filters, id, function):
    ...
    return (
        '\n        <label class="checkBtn%s"><input type="checkbox" onclick="%s(this)"'
        ' id="%s"%s/>%s</label>'
        % (
            " disabled" if not enabled else "",
            function,
            id,
            "checked" if enabled else "disabled",
            id,
        )
    )
...
def filter_bar(enabled):
    return "".join(
        [
            ...
            "".join(
                [
                    filter_button(enabled, severity, "toggleSeverity")
                    for severity in [
                        "error",
                        "warning",
                        "portability",
            ...
            "\n        | ",
            "".join(
                [filter_button(enabled, tool, "toggleTool") for tool in ["cppcheck", "clang-tidy"]]
            ),
            "\n        | ",
            (
                '\n        <label class="severityHeader">File: <input type="text"'
                ' oninput="filterFile(this.value)"/></label>'
            ),
            (
                '\n        <label class="severityHeader">Filter: <input type="text"'
                ' oninput="filterText(this.value)"/></label>'
            ),
            "\n      </div>\n",
        ]
    )
                            ...
                            if error["inconclusive"] == "true":
                                ...
                                if error.get("verbose") and (error["verbose"] != error["msg"]):
                                    ...
                                    t = (
                                        ...
                                        % (
                                            error["msg"],
                                            html_escape(error["verbose"].replace("\\012", "\n")),
                                        )
                                        + t[index + 1 :]
                                    )
                                else:
    ...
    def handleVersion2(self, name, attributes):
        ...
        if name == "error":
            ...
            if not locations:
                error["file"] = file
                error["line"] = line
            locations.append({"file": file, "line": line, "info": attributes.get("info")})
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--title", dest="title", help="The title of the project.", default="[project name]"
    )
    parser.add_option(
        "--file",
        dest="file",
        action="append",
        help=(
            "The cppcheck xml output file to read defects "
    ...
    parser.add_option(
        "--report-dir",
        dest="report_dir",
        help="The directory where the HTML report content is written.",
    )
    parser.add_option(
        "--source-dir",
        dest="source_dir",
        help="Base directory where source code files can be found.",
    )
    parser.add_option(
        "--add-author-information",
        dest="add_author_information",
        help=(
            "Blame information to include. "
    ...
    parser.add_option(
        ...
        dest="blame_options",
        help=(
            "[-w, -M] blame options which you can use to get author and author mail  "
    ...
    if options.add_author_information:
        ...
        for x in fields:
            if x.lower() in ["n", "name"]:
                ...
                print(
                    'Unrecognized value "%s" for author information, using default (name, email,'
                    " date)" % x
                )
    ...
    for filename, data in sorted(files.items()):
        ...
        htmlFormatter = AnnotateCodeFormatter(
            linenos=True,
            style="colorful",
            hl_lines=lines,
            lineanchors="line",
        ...
        with io.open(
            os.path.join(options.report_dir, htmlfile), "w", encoding="utf-8"
        ) as output_file:
            output_file.write(
                HTML_HEAD
                % (
            ...
            for error in sorted(errors, key=lambda k: k["line"]):
                output_file.write(
                    '<a href="%s#line-%d"> %s %s</a>'
                    % (data["htmlfile"], error["line"], error["id"], error["line"])
                )
            ...
            try:
                ...
                try:
                    ...
                    sys.stderr.write(
                        "ERROR: Couldn't determine lexer for the file' "
                        + source_filename
                        + " '. Won't be able to syntax highlight this file."
                    )
                    output_file.write(
                        '\n <tr><td colspan="5"> Could not generate content because pygments failed'
                        " to determine the code type.</td></tr>"
                    )
            ...
            output_file.write(
                highlight(content, lexer, htmlFormatter).decode(options.source_encoding)
            )
    ...
    with io.open(os.path.join(options.report_dir, "index.html"), "w") as output_file:
        ...
        for filename, data in sorted(files.items()):
            for error in data["errors"]:
                ...
                filter_enabled[
                    "clang-tidy" if error["id"].startswith("clang-tidy-") else "cppcheck"
                ] = True
                stats_count += 1
        ...
        stat_fmt = (
            '\n            <tr><td><input type="checkbox" class="idToggle"'
            ' onclick="toggleDisplay(this)" id="{}" name="{}"'
            " checked></td><td>{}</td><td>{}</td></tr>"
        )
        ...
        output_file.write(HTML_HEAD_END)
        output_file.write(
            HTML_MENU.replace('id="menu"', 'id="menu_index"', 1).replace(
                "Defects:", "Defect summary", 1
            )
            % ""
        )
        output_file.write(
            '\n       <label><input type="checkbox" class="idToggle" onclick="toggleAll()" checked>'
            " Toggle all</label>"
        )
        output_file.write("\n       <table>")
        ...
        for filename, data in sorted(files.items()):
            ...
            row_content = (
                filename if file_error else '<a href="%s">%s</a>' % (data["htmlfile"], filename)
            )
            ...
            blame_data = (
                git_blame(sorted_errors, source_dir, filename, blame_options)
            ...
            for error in sorted_errors:
                ...
                output_file.write(
                    ...
                    % tr_str(
                        ...
                        error["severity"],
                        error["msg"],
                        git_blame_dict.get("author", "Unknown"),
                        git_blame_dict.get("author-mail", "---"),
                        git_blame_dict.get("author-time", "---"),
                        tr_class=to_css_selector(error["id"])
                        + " sev_"
                        + error["severity"]
                        + " issue",
    ...
    if decode_errors:
        ...
        sys.stderr.write(
            '\nConsider changing source-encoding (for example: "htmlreport ...'
    ...
    with io.open(os.path.join(options.report_dir, "stats.html"), "w") as stats_file:
        ...
        for sev in SEVERITIES:
            ...
            try:
                for filename in stats_countlist:
                    try:  # also bail out if we have a file with no sev-results
                        ...
                        stats_templist[filename] = int(
                            stats_countlist[filename][sev]
                        )  # file : amount,
                    except KeyError:
                        continue
                ...
                continue
            stats_file.write(
                '<p><span class="statHeader">Top 10 files for '
                + sev
            ...
            stats_list_sorted = sorted(
                stats_templist.items(), key=operator.itemgetter(1, 0), reverse=True
            )
            it = 0
            LENGTH = 0
            ...
            for i in stats_list_sorted:  # printing loop
                ...
                if it == 0:
                    LENGTH = len(
                        str(i[1])
                    )  # <- length of longest number, now get the difference and try to  make other numbers align to it

                stats_file.write(
                    "&#160;" * 3
                    + str(i[1])
                    + "&#160;" * (1 + LENGTH - len(str(i[1])))
                    + '<a href="'
                    + files[i[0]]["htmlfile"]
                    + '">  '
                    + i[0]
