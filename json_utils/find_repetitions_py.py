...
def main():
    parser = argparse.ArgumentParser(description='Compare error codes in JSON files for a specific file.')
    ...
    all_codes = [entry['code'] for filename in os.listdir(source_directory) 
                 if filename.endswith('.json') 
                 for entry in load_json(os.path.join(source_directory, filename)).get("./files/" + os.path.splitext(filename)[0] + ".py", [])]
